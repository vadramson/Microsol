...
//        print_r($regn); print ("19");
    ...
    Public function Print_travels($travel) {
        ...
        $re = $this->_db->query("SELECT prsid FROM  employee, travel_order WHERE travel_order.trempcode = employee.ecode ") or die(mysql_error());    
        $rcg = $re->fetch();
        $rc = $rcg["prsid"];
        $re1 = $this->_db->query("SELECT trempcode FROM  travel_order, employee WHERE trempcode = employee.ecode ") or die(mysql_error());    
        $rc1 = $re1->fetch();
        
        $pr = $this->_db->query("SELECT * FROM person, employee WHERE person.prsid = '".$rc."' ") or die(mysql_error());    
        $per = $pr->fetch();
