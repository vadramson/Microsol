...
class AssetsManager{
    ...
    public function buyAssets($assets) {
//         echo "boy";
        $resul = $this->_db->query("SELECT * FROM assets WHERE ascode = '" .$assets->cycode() ." ' ") or die(mysql_error());
        $reqt = $resul->fetch();
//         echo "boy1";
        if ($reqt == NULL) {
            
            $q = $this->_db->prepare('INSERT INTO assets SET  empid = :empid, ascode = :cycode, dop = :dop, asname = :ramount, asamount = :gamount, name = :cusname, nic = :nic, phone = :exchcode, status = :phone ') or die(mysql_error());
      ...
      public function saleAssets($assets){
//         echo "boy";
        $resul = $this->_db->query("SELECT * FROM sassets WHERE ascode = '" .$assets->ascode() ." ' ") or die(mysql_error());
        $reqt = $resul->fetch();
//         echo "boy1";
        if ($reqt == NULL) {
            
            $q = $this->_db->prepare('INSERT INTO sassets SET  empid = :empid, ascode = :ascode, dop = :dop, asname = :asname, asamount = :asamount, name = :name, nic = :nic, phone = :phone, transcode = :transcode ') or die(mysql_error());
            $q-> bindValue(':empid', $assets->empid());
            ...
            echo"<script language='javascript'>alert('Assset sold already')</script>";
        }
   ...
   function listAssets($page, $per_page) {
...
//        $resul = $this->_db->query("SELECT town.*, count(bchid) AS nbbranch FROM town LEFT JOIN branch ON town.twnid=branch.twnid GROUP BY twnid ") or die(mysql_error());
        ...
        $resul = $this->_db->query("SELECT * FROM sassets LIMIT {$start},{$per_page} ") or die(mysql_error());
        ...
        while ($rec = $resul->fetch()) {
            ...
            echo" 
                 <td>" . $rec["transcode"]. "</td> <td>" . $rec["ascode"] . "</td><td>" . $rec["asname"] . "</td><td>" . $rec["asamount"]. ' F CFA' . "</td><td>" .$rec["name"] . "</td><td>". $rec["phone"] . "</td><td>" . $rec["dop"] . "</td>
                   <td> 
                 <a href=../VIeW/indexadmin.php?page=" . base64_encode('../VIeW/AssetsVIeW/printAssetsHistory_pdf') . "&idr=" . base64_encode($rec["sasid"]) . " target = blank>   
...
//        print_r($regn); print ("19");
    ...
    Public function Print_AssetHitory($history) {
        
        $resul = $this->_db->query("SELECT * FROM  sassets WHERE sasid = '" . $history->sasid() . "' ") or die(mysql_error());    
        ...
        &nbsp;&nbsp;&nbsp; _____________________________________________________________-->
             <div style="width: 100%; background-color: #fbe9e7; ">
                <div style="width: 67%; border: 2px; text-align: left; padding-top: -33m;">
                    ...
                    <p> <strong> <span style=" margin-left: 1mm;"> Amount :</span> </strong> <span style="font-family: Arial; font-size: 10pt;;"> <?php echo $rec["asamount"]." F CFA" ?></span> </p> 
                    <p> <strong> <span style=" margin-left: 1mm;"> Asset's Name :</span> </strong> <span style="font-family: Arial; font-size: 10pt;"> <?php echo $rec["asname"];?></span> </p> 
                    <p> <strong> <span style=" margin-left: 1mm;"> Asset's Code:</span> </strong> <span style="font-family: Arial; font-size: 10pt;"> <?php echo $rec["ascode"];?></span> </p>                  
